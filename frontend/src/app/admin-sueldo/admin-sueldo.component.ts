...
import { TablasMaestrasService, TaSueldo, Estudio } from '../services/tablas-maestras.service';
import { ConfirmDialogService } from '../services/confirm-dialog.service';

interface AyudaContextual { titulo: string; descripcion: string; puntos?: string[]; }

@Component({
  selector: 'app-admin-sueldo',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './admin-sueldo.component.html'
})
export class AdminSueldoComponent implements OnInit {
  ...
  readonly ayudas: Record<string, AyudaContextual> = {
    general: {
      titulo: 'Sueldo',
      descripcion: 'Administra el importe del sueldo base por año y tipo de docencia.',
    ...
    codEstudio: { titulo: 'Tipo de Docencia', descripcion: 'Tipo de docencia al que aplica el sueldo.' },
    importe: { titulo: 'Importe', descripcion: 'Importe mensual del sueldo base en euros. Debe ser mayor que cero.' }
  };
  ...
  cargarDatos(): void {
    this.service.getSueldo().subscribe({
      next: data => {
  ...
  alta(): void {
    ...
    this.service.createSueldo(this.formulario as TaSueldo).subscribe({
  ...
  clave(r: TaSueldo): string { return `${r.anio}_${r.codEstudio}`; }
  guardarEdicion(r: TaSueldo): void {
    const val = this.importeEdicion[this.clave(r)];
    if (!val || +val <= 0) { this.mostrarError('El importe debe ser mayor que cero'); return; }
    this.service.updateSueldo(r.anio, r.codEstudio, { importe: +val }).subscribe({
