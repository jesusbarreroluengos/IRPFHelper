...
import { TablasMaestrasService, TaTrienio, Estudio } from '../services/tablas-maestras.service';
import { ConfirmDialogService } from '../services/confirm-dialog.service';

interface AyudaContextual { titulo: string; descripcion: string; puntos?: string[]; }

@Component({
  selector: 'app-admin-trienio',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './admin-trienio.component.html'
})
export class AdminTrienioComponent implements OnInit {
  ...
  registros: TaTrienio[] = [];
  estudios: Estudio[] = [];
  formulario: Partial<TaTrienio> = {};
  importeEdicion: { [key: string]: number | null } = {};

  filtroAnio: number | '' = '';
  filtroCodEstudio: string = '';

  get registrosFiltrados(): TaTrienio[] {
  ...
  readonly ayudas: Record<string, AyudaContextual> = {
    general: {
      titulo: 'Trienios',
      descripcion: 'Administra el importe de trienios por año y tipo de docencia.',
    ...
    codEstudio: { titulo: 'Tipo de Docencia', descripcion: 'Tipo de docencia al que aplica el trienio.' },
    importe: { titulo: 'Importe', descripcion: 'Importe mensual del trienio en euros. Debe ser mayor que cero.' }
  };
  ...
  cargarDatos(): void {
    this.service.getTrienio().subscribe({
      next: data => {
  ...
  alta(): void {
    ...
    this.service.createTrienio(this.formulario as TaTrienio).subscribe({
  ...
  clave(r: TaTrienio): string { return `${r.anio}_${r.codEstudio}`; }

  guardarEdicion(r: TaTrienio): void {
    const val = this.importeEdicion[this.clave(r)];
    if (!val || +val <= 0) { this.mostrarError('El importe debe ser mayor que cero'); return; }
    this.service.updateTrienio(r.anio, r.codEstudio, { importe: +val }).subscribe({
