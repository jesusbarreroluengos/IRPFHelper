...
import { TablasMaestrasService, TaDestino, Estudio } from '../services/tablas-maestras.service';
...
@Component({
  ...
  templateUrl: './admin-destino.component.html'
})
export class AdminDestinoComponent implements OnInit {
  ...
  registros: TaDestino[] = [];
  estudios: Estudio[] = [];
  formulario: Partial<TaDestino> = {};
  importeEdicion: { [key: string]: number | null } = {};

  filtroAnio: number | '' = '';
  filtroCodEstudio: string = '';

  get registrosFiltrados(): TaDestino[] {
  ...
  readonly ayudas: Record<string, AyudaContextual> = {
    general: {
      titulo: 'Complemento de Destino',
      descripcion: 'Administra el importe del complemento de destino por año y tipo de docencia.',
      puntos: [
        'Solo puede existir un registro por combinación de año y tipo de docencia',
    ...
    importe: { titulo: 'Importe', descripcion: 'Importe mensual del complemento de destino en euros. Debe ser mayor que cero.' }
  };
  ...
  cargarDatos(): void {
    this.service.getDestino().subscribe({
      next: data => {
  ...
  alta(): void {
    ...
    if (!this.formulario.importe || +this.formulario.importe <= 0) { this.mostrarError('El importe es obligatorio y debe ser mayor que cero'); return; }
    this.service.createDestino(this.formulario as TaDestino).subscribe({
  ...
  clave(r: TaDestino): string { return `${r.anio}_${r.codEstudio}`; }

  guardarEdicion(r: TaDestino): void {
    const val = this.importeEdicion[this.clave(r)];
    if (!val || +val <= 0) { this.mostrarError('El importe debe ser mayor que cero'); return; }
    this.service.updateDestino(r.anio, r.codEstudio, { importe: +val }).subscribe({
